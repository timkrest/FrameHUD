...
`exportSession` записывает сессию с последнего сброса — статистику, окно кадров с разбивкой по
фазам, худшие кадры с временем по часам, контекст, устройство, версию приложения, состояние
измерений и проблемы достоверности — как JSON и самодостаточный HTML-отчёт, и возвращает оба файла. Они ложатся в
