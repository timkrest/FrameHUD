...
окно кадров, худшие кадры с временем по часам, контекст, устройство, версия приложения, состояние
измерений и проблемы достоверности. Они ложатся в `framehud/` во внешнем каталоге файлов приложения,
так что CI забирает их без root; `shareSession` открывает с ними системное окно «Поделиться». Ничего
...
extras очищает контекст. `EXPORT` отвечает путём к отчёту в результате броадкаста, сразу готовым для
...
тормозил две секунды, к моменту чтения снова выглядит нормально. Здесь же учтён каждый кадр,
собранный интервалом. Оба отчёта несут разбивку по сессии и по экрану, а HTML-отчёт ставит их рядом.
...
что упавший тест остаётся со своей ошибкой. Итоги сессии переживают закрытие панели, поэтому цифры есть
