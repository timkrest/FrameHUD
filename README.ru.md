...
называет строку, на которую стоит смотреть, а `◀` её помечает.
...
`adb pull`, а `BASELINE` — путём к обновлённому базлайну. Сборка без флага отладки игнорирует все
команды.
...
Команда усредняет текущую сессию в `framehud/baseline.json` и отвечает путём к нему. Второй вызов
без `RESET` между ними ничего не меняет, так что повтор команды не засчитает один прогон дважды.
