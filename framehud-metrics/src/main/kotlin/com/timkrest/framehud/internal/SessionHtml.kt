...
private fun HtmlScope.incidents(snapshot: SessionSnapshot) = with(snapshot) {
    ...
    section("Incidents") {
        for (incident in incidents) {
            ...
            mainThreadBlock(incident.worst.mainThreadBlock)
        }
    }
}
...
private fun incidentDetails(worst: IncidentWindow): String {
    ...
    ).plus(worst.counters.map { "${it.name} ${it.value}" }).joinToString(INCIDENT_DETAIL_SEPARATOR)
}

private fun HtmlScope.mainThreadBlock(block: MainThreadBlock) {
    if (block.calls.isEmpty()) return
    meta("Main thread blocked ${block.durationMs} ms, ${block.stacksTaken} stacks taken")
    list {
        for (call in block.calls) item("${call.samples}x ${call.name}")
    }
}
...
private fun HtmlScope.counters(snapshot: SessionSnapshot) = with(snapshot) {
    if (counters.isEmpty()) return@with
    section("Counters") {
        table {
            for (counter in counters) {
                val peak = counter.peakSinceReset.takeIf { it > counter.value }
                row(counter.name, peaked("${counter.value}", peak?.toString()))
            }
        }
    }
}
