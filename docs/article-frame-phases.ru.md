# Как понять, какая стадия кадра тормозит приложение
...
баг отрисовки. По проценту джанка — доле кадров, которые не уложились в срок, — они неразличимы.

Экран — список из примера к FrameHUD, отладочной панели, которая показывает разбивку кадра по
фазам, как её отдаёт Android. Три версии этого экрана: как написан, с шестью миллисекундами
`Thread.sleep` внутри `drawBehind` и с шестьюдесятью полупрозрачными слоями поверх каждой строки.
Каждая колонка — среднее двух скриптованных 36-секундных скроллов на Galaxy S25 Ultra при 120 Гц.
...
Сначала правая колонка. Джанк 4.3% против 3.8% на чистом прогоне — 0.44 пункта, примерно столько
же, на сколько расходятся два прогона одного и того же экрана с overdraw. Потерянного времени
*меньше*, чем на чистом. На этом телефоне такой экран уезжает в релиз.
...
Оба бага мои — переключатели нагрузки в примере, потому методика выписана, а не пересказана. Android
16, дисплей прибит к 120 Гц, один и тот же скриптованный инерционный скролл каждый раз, около 36
секунд и ~3300 кадров на прогон, сессия экспортируется после каждого. Ни один прогон не отметился
проблемой достоверности. Два чистых прогона разошлись на 0.06 пункта джанка, два прогона с
overdraw — на 0.37, а два прогона со `sleep` — на 1.3, что уже шире разрыва между чистой колонкой и
колонкой с overdraw. Доказывает таблица нечто более узкое, чем «библиотека работает»: разбивка кадра
по фазам разделяет два случая, которые счётчик кадров показывает одинаковыми.
...
![Дедлайны тридцати кадров против фиксированного бюджета 16.7 мс](frame-budget.svg)
...
`SYSTEM_ALERT_WINDOW`. Единственная строка, которую релизная сборка может взять, — `framehud-noop`,
зеркало API с пустыми телами, и только если `FrameHud` вызывается вне `src/debug`: этим строкам
всё ещё надо компилироваться.
