...
Everything FrameHUD does once the dependency is in place.
...
```kotlin
qaImplementation("com.timkrest:framehud-metrics:0.18.0")
```

`FrameHud` is the same object, so the code around it stays as it is. `enabled` switches collection
alone and `overlayMode` is ignored. `framehud` is this artifact, the panel and
[the adb commands](#driving-over-adb), so a debug build needs nothing else. On its own this one
declares no exported component at all.

Such a build does not have to be debuggable. Collection, events, exports and the baseline all work
on a release-signed QA flavour. What stays behind `android:debuggable` is the `run-as` fallback
[an export](#exporting-a-session) needs on a device with no external storage.
...
| `eventListeners` | `[LogcatEventListener]` | Who receives first-frame, usable-frame, jank, frozen-frame, thermal, screen, interaction and internal-failure events. |
...
A main thread that goes 300 ms without handling a frame is sampled from a background thread there
and then, again 100 ms later, and after that at twice the wait each time up to 800 ms, until it
draws again. The incident keeps the calls those samples caught it in most often, so a frozen frame
names the work that held the thread and not only how long it was held. A stack taken more than two
seconds before the trigger explains nothing about it and is left out. The JSON has it as
`mainThreadBlock`, and the HTML report lists it under the incident's chart.
...
An app can ask at a moment of its own with `FrameHud.retainTrace()`, and QA with the `RETAIN`
broadcast from [the adb set](#driving-over-adb). Asking again within five seconds changes nothing,
so a burst of incidents costs one ask rather than one per frame. The session report names the
trigger and counts the asks, so a run that retained nothing does not look like one that was never
wired up.
...
```kotlin
qaImplementation("com.timkrest:framehud-qa:0.18.0")
```
...
The receiver has to be exported, because `adb shell am broadcast` cannot reach it otherwise, and it
requires the sender to hold `android.permission.DUMP`. The shell holds it; an app a user installs
cannot be granted it. A build that wants the panel without the commands drops the receiver:
